...
const { setupModePrompt, setupModeKeyboard } = require('./src/mode-picker');
...
const parseDateTime = (dateText, timeText = '12:00') => { const date = dateText ? new Date(`${dateText}T${timeText || '12:00'}`) : new Date(Date.now() + 24 * 60 * 60 * 1000); if (!dateText) date.setHours(12, 0, 0, 0); return date.toISOString(); };
function formatPlan(plan, currency = 'USD', locale = 'en') { const joined = plan.participants.filter((p) => p.status === 'joined'); const when = new Date(plan.startsAt); const date = Number.isNaN(when.getTime()) ? plan.startsAt : when.toLocaleString(locale, { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }); return `${plan.title}\n${date}${plan.location ? ` · ${plan.location}` : ''}\n${joined.length} joined · ${plan.costMode === 'shared' ? `Shared Cost${plan.estimatedBudgetCents ? ` · ${money(plan.estimatedBudgetCents, currency, locale)} est.` : ''}` : 'Free'}\n${plan.id}`; }
function planListMessage(ctx) { const locale = localeFor(ctx); const data = store.dashboard(ctx.chat.id); const actor = actorName(ctx); const active = data.plans.filter((plan) => plan.status === 'active').sort((a, b) => new Date(a.startsAt) - new Date(b.startsAt)); if (!active.length) return 'No active plans yet.\nCreate one in the Mini App or use /plan create Saturday BBQ date=2026-08-15 time=17:00'; return `Plans\n\n${listByNumber(active, (plan) => `${formatPlan(plan, data.settings.currency, locale)}${plan.participants.some((p) => p.status === 'joined' && p.displayName.toLowerCase() === actor.toLowerCase()) ? '\nJoined ✓' : ''}`)}`; }
...
bot.command('setup', (ctx) => { const locale = localeFor(ctx); const input = commandInput(ctx, 'setup'); const current = store.settings(ctx.chat.id); if (!input) return ctx.reply(`${setupModePrompt(current.cribMode)}\n\nUse /setup <house name> to name this crib.`, { reply_markup: setupModeKeyboard(current.cribMode) }); const updated = store.updateSettings(ctx.chat.id, { houseName: input }, actorName(ctx)); return ctx.reply(`Setup saved\nHouse: ${updated.houseName}\nMode: ${setupModePrompt(updated.cribMode).split('\n')[0]}`, { reply_markup: setupModeKeyboard(updated.cribMode) }); });
bot.action(/^setup:mode:([a-z0-9_-]+)$/, async (ctx) => { const mode = ctx.match[1]; const updated = store.updateSettings(ctx.chat.id, { cribMode: mode }, actorName(ctx)); await ctx.answerCbQuery(`Mode set to ${updated.cribMode}`); return ctx.editMessageText(`${setupModePrompt(updated.cribMode)}\n\nUse /setup <house name> to name this crib.`, { reply_markup: setupModeKeyboard(updated.cribMode) }); });
...
bot.command('plans', (ctx) => ctx.reply(planListMessage(ctx), { reply_markup: dashboardKeyboard(ctx, 'plans', localeFor(ctx)) }));
bot.command('plan', (ctx) => {
  const locale = localeFor(ctx); const input = commandInput(ctx, 'plan'); const [action, ...rest] = input.split(/\s+/); const actor = actorName(ctx); const profile = store.memberByTelegramId(ctx.chat.id, ctx.from.id);
  if (!action) return ctx.reply(planListMessage(ctx), { reply_markup: dashboardKeyboard(ctx, 'plans', locale) });
  if (action.toLowerCase() === 'create') {
    const raw = rest.join(' '); const params = Object.fromEntries([...raw.matchAll(/(\w+)=("[^"]+"|\S+)/g)].map((m) => [m[1], m[2].replace(/^"|"$/g, '')]));
    const title = params.title || raw.replace(/\s+\w+=("[^"]+"|\S+)/g, '').trim(); if (!title) return ctx.reply('Format: /plan create Saturday BBQ date=2026-08-15 time=17:00 cost=shared');
    try { const plan = store.addPlan(ctx.chat.id, { title, type: params.type || 'Custom', customType: params.customType, location: params.location || '', description: params.description || '', startsAt: parseDateTime(params.date, params.time), endsAt: params.endsAt || null, costMode: ['shared', 'cost', 'shared-cost'].includes(String(params.cost || params.costMode || '').toLowerCase()) ? 'shared' : 'free', estimatedBudgetCents: params.budget ? Math.round(Number(params.budget.replace(',', '.')) * 100) : null, bringItems: params.bring ? params.bring.split(',').map((name) => ({ name: name.trim() })) : [], createdByTelegramId: ctx.from.id }, actor, profile); return ctx.reply(`Plan created\n${formatPlan(plan, store.settings(ctx.chat.id).currency, locale)}`, { reply_markup: dashboardKeyboard(ctx, 'plans', locale) }); } catch (error) { return ctx.reply(error.message); }
  }
  if (action.toLowerCase() === 'join') { const result = store.joinPlan(ctx.chat.id, rest[0], actor, profile); return ctx.reply(result ? `Joined ✓ ${result.plan.title}` : 'Could not join that active plan.', { reply_markup: dashboardKeyboard(ctx, 'plans', locale) }); }
  if (action.toLowerCase() === 'leave') { const result = store.leavePlan(ctx.chat.id, rest[0], actor, profile); return ctx.reply(result?.left ? `Left ${result.plan.title}` : 'You were not joined to that plan.'); }
  if (action.toLowerCase() === 'item') { try { const result = store.addPlanItem(ctx.chat.id, rest[0], { name: rest.slice(1).join(' ') }, actor, profile); return ctx.reply(result ? `Bring item added\n${result.item.name}` : 'Could not find that active plan.'); } catch (error) { return ctx.reply(error.message); } }
  if (['claim', 'unclaim'].includes(action.toLowerCase())) { const result = store.claimPlanItem(ctx.chat.id, rest[0], rest.slice(1).join(' '), actor, profile, action.toLowerCase() === 'claim'); return ctx.reply(result ? `${action.toLowerCase() === 'claim' ? 'Claimed' : 'Unclaimed'} ${result.item.name}` : 'Could not find that plan item.'); }
  if (action.toLowerCase() === 'expense') { const [planId, amountText, ...descriptionParts] = rest; const amount = Number(amountText?.replace(',', '.')); if (!planId || !Number.isFinite(amount) || amount <= 0) return ctx.reply('Format: /plan expense <plan-id-or-number> 20 snacks'); try { const expense = store.addPlanExpense(ctx.chat.id, planId, { amount, description: descriptionParts.join(' ') || 'Plan expense', paidBy: actor, category: 'Plans' }, actor, 'telegram', profile); return ctx.reply(expense ? `Plan expense added\n${expenseLine(expense, store.settings(ctx.chat.id).currency, locale)}` : 'Could not find that active shared-cost plan.'); } catch (error) { return ctx.reply(error.message); } }
  if (['complete', 'cancel'].includes(action.toLowerCase())) { try { const plan = store.updatePlanStatus(ctx.chat.id, rest[0], action.toLowerCase() === 'complete' ? 'completed' : 'cancelled', actor, profile); return ctx.reply(plan ? `Plan ${plan.status}\n${plan.title}` : 'Could not find that plan.'); } catch (error) { return ctx.reply(error.message); } }
  const plan = store.findPlan(ctx.chat.id, action); if (!plan) return ctx.reply('Plan commands: /plan, /plan create, /plan join <plan>, /plan leave <plan>, /plan item <plan> Ice, /plan claim <plan> Ice, /plan expense <plan> 20 snacks');
  return ctx.reply(formatPlan(plan, store.settings(ctx.chat.id).currency, locale));
});
...
async function performAction(chatId, action, payload, viewer) {
  ...
  if (action === 'fund.create') return store.addFund(chatId, { title: payload.title, target: Number(payload.goal ?? payload.target) }, actor);
  if (action === 'fund.chipin') { const result = store.chipInFund(chatId, payload.fundId, Number(payload.amount), actor); if (!result) throw Object.assign(new Error('Fund not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.create') return store.addPlan(chatId, { ...payload, estimatedBudgetCents: payload.estimatedBudgetCents ?? (payload.estimatedBudget ? Math.round(Number(payload.estimatedBudget) * 100) : null), createdByTelegramId: viewer.telegramId }, actor, viewer);
  if (action === 'plan.join') { const result = store.joinPlan(chatId, payload.id, actor, viewer); if (!result) throw Object.assign(new Error('Plan not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.leave') { const result = store.leavePlan(chatId, payload.id, actor, viewer); if (!result) throw Object.assign(new Error('Plan not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.item.add') { const result = store.addPlanItem(chatId, payload.planId, payload, actor, viewer); if (!result) throw Object.assign(new Error('Plan not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.item.claim') { const result = store.claimPlanItem(chatId, payload.planId, payload.itemId, actor, viewer, true); if (!result) throw Object.assign(new Error('Plan item not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.item.unclaim') { const result = store.claimPlanItem(chatId, payload.planId, payload.itemId, actor, viewer, false); if (!result) throw Object.assign(new Error('Plan item not found.'), { statusCode: 404 }); return result; }
  if (action === 'plan.complete' || action === 'plan.cancel') { const plan = store.updatePlanStatus(chatId, payload.id, action === 'plan.complete' ? 'completed' : 'cancelled', actor, viewer); if (!plan) throw Object.assign(new Error('Plan not found.'), { statusCode: 404 }); return plan; }
  if (action === 'plan.expense.add') { const expense = store.addPlanExpense(chatId, payload.planId, { amount: Number(payload.amount), description: payload.description, paidBy: payload.paidBy || actor, category: 'Plans', notes: payload.notes }, actor, 'app', viewer); if (!expense) throw Object.assign(new Error('Plan not found.'), { statusCode: 404 }); return expense; }
